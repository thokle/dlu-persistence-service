   ...
   public class SendTilNavisionService
    {
        ...
        public Tuple<string, string> SendTilnavison(int indrykingsUge)
        {
            ...
            try
            {
                ...
                entities.Database.ExecuteSqlCommand(@"UPDATE dbo.tblMedieplanLinjer SET TotalPris = (SELECT TOP (1) TotalPris FROM dbo.tblMedieplanLinjer AS tblMedieplanLinjer_1 WHERE     (MedieplanNr = dbotblMedieplanLinjer.MedieplanNr) AND (TotalPris IS NOT NULL) AND (Version < tblMedieplanLinjer.Version) AND (UgeavisID = tblMedieplanLinjer.UgeavisID) ORDER BY Version DESC) WHERE (TotalPris IS NULL) AND (Version > 11)");
...
FROM dbo.tblMedieplan AS tblMedieplan_1 INNER JOIN dbo.tblMedieplanNr ON tblMedieplan_1.MedieplanNr = dbo.tblMedieplanNr.MedieplanNr AND tblMedieplan_1.Version = dbo.tblMedieplanNr.AktivVersion WHERE (dbo.tblMedieplanNr.Status = 6) AND (dbo.tblMedieplanNr.AktivVersion < 100) AND (tblMedieplan_1.IndrykningsUge = @indrykingsUge ", new SqlParameter("indrykingsUge", indrykingsUge));
                ...
                entities.Database.ExecuteSqlCommand(@"INSERT INTO tblMedieplanLinjer (MedieplanNr, Version, UgeavisID, Mm, NormalMmPris, MmPris, MmRabat, MåGiveMmRabat, MmTotal, FarveTillæg, FarvePris, FarveRabat, FarveMin, FarveMax, FarveTotal, Total, MåGiveMaterialeGodtgørelse, SkalGiveMaterialeGodtgørelse," +
